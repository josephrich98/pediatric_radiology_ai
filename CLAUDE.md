...
- Task categories are defined by what the model *produces* (label/box, mask,
  risk, better image, text, pretrained backbone, synthetic image, autonomous
  action, workflow decision); `config.TASK_GLOSS` carries the one-line
  meaning shown on the slides.
...
- Slides are regenerated by `scripts/build_slides.py`; the "what the top
  papers answer" tables use citation/star thresholds set at the top of that
  script and fall back to a title heuristic for papers not in
...
- To refresh to today, run `python scripts/refresh.py` (clears the cache except
  ...
  images, then figures, reports, slides, and
  latexmk; `--quick`, `--keep-cache`, `--no-slides`, `--skip <collector>`,

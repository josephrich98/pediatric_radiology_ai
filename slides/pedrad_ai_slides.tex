...
\institute{CHLA Radiology Grand Rounds}
\date{October 8, 2026}
...
\begin{itemize}
  \item Describe the big players in radiology AI, both general and pediatric
...
\vspace{2pt}
{\small\textbf{Date range}: January 1 2008 -- September 30 2026\par}
...
\begin{frame}[category=Conferences]{MICCAI: pediatric radiology AI works, 2024--2026}
\tiny
...
\begin{frame}[category=Conferences]{MIDL: pediatric radiology AI works, 2023--2026}
\tiny
\begin{adjustbox}{max totalsize={\textwidth}{0.66\textheight}}
\renewcommand{\arraystretch}{1.2}
...
\begin{center}\includegraphics[height=0.82\textheight,width=\textwidth,keepaspectratio]{commercial_problems.png}\end{center}
\end{frame}

\begin{frame}[category=Commercial Software]{Products a pediatric radiologist should know (part 1)}
\tiny
\begin{adjustbox}{max totalsize={\textwidth}{0.72\textheight}}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{|>{\raggedright\arraybackslash}p{0.150\textwidth}|>{\raggedright\arraybackslash}p{0.130\textwidth}|>{\raggedright\arraybackslash}p{0.176\textwidth}|>{\raggedright\arraybackslash}p{0.378\textwidth}|>{\raggedright\arraybackslash}p{0.324\textwidth}|>{\raggedright\arraybackslash}p{0.176\textwidth}|}
\hline \textbf{Product} & \textbf{Vendor} & \textbf{Clinical problem} & \textbf{What it does} & \textbf{Pediatric scope} & \textbf{Where it stands} \\ \hline
...
\end{tabular}
\end{adjustbox}
\\[3pt]{\scriptsize Square brackets give the FDA submission supporting the pediatric claim and its decision year; a blank means no US authorization was identified for that product. ``Where it stands'' is our reading of deployment, not a measured adoption rate. A company's total of FDA AI entries is not a pediatric credential --- the large vendors' entries are mostly whole scanners whose labels list pediatric imaging as one clinical application (see the authorizations-per-company figure).}
\end{frame}

\begin{frame}[category=Commercial Software]{Products a pediatric radiologist should know (part 2)}
\tiny
\begin{adjustbox}{max totalsize={\textwidth}{0.68\textheight}}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{|>{\raggedright\arraybackslash}p{0.150\textwidth}|>{\raggedright\arraybackslash}p{0.130\textwidth}|>{\raggedright\arraybackslash}p{0.176\textwidth}|>{\raggedright\arraybackslash}p{0.378\textwidth}|>{\raggedright\arraybackslash}p{0.324\textwidth}|>{\raggedright\arraybackslash}p{0.176\textwidth}|}
\hline \textbf{Product} & \textbf{Vendor} & \textbf{Clinical problem} & \textbf{What it does} & \textbf{Pediatric scope} & \textbf{Where it stands} \\ \hline
...
Butterfly iQ3; Clarius Bladder AI & Butterfly Network / Clarius & point-of-care ultrasound & Handheld ultrasound with on-device AI: bladder volume, gestational age, ejection fraction, lung and nerve tools & The scanners are labeled for adult and pediatric imaging, but each AI tool carries its own age scope and most are adult-only; Clarius Bladder AI covers ages 2 and over [K232257, 2023] & Scanners widely deployed; AI tools mostly adult-scoped \\ \hline
...
\begin{frame}[category=Reviews]{An independent scoping review of pediatric radiology AI}
\relax
{\scriptsize Kamran et al., \textit{Pediatric Radiology} 2026
(\href{https://doi.org/10.1007/s00247-025-06462-5}{doi 10.1007/s00247-025-06462-5})\par}
\medskip
\small
\begin{itemize}
...
\begin{frame}[category=Reviews]{ACR white paper: the pediatric AI gap}
\relax
...
\begin{frame}[category=Reviews]{Multisociety statement: safe pediatric AI implementation}
\relax
...
\begin{frame}{Worth knowing as a radiologist}
\scriptsize
\begin{itemize}
\item \textbf{Deep Learning: A Primer for Radiologists} (Radiographics paper, 2017): a helpful introduction to the principles and applications of deep learning in radiology; essential for understanding how AI tools work and their limitations
...
\begin{itemize}
  ...
  \item AI development is constrained by scarce public datasets, fragmented institutional data, rare diseases, and heterogeneous reporting practices.
